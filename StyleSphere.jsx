...
import {
  ...
  ArrowLeft, KeyRound, CircleDot, Send, Loader2, ArrowRight, RotateCcw, PanelLeft, Mail, Phone, ShieldCheck, MessageSquare
} from "lucide-react";
import {
...
function CommunicationPanel({ vendor, tall }) {
  const [messages, setMessages] = useState(
    MESSAGE_SEEDS[vendor.id] || [
      { from: "admin", text: `Hi ${vendor.name.split(" ")[0]}, welcome to StyleSphere! Let us know if you have any questions during onboarding.`, time: "2 days ago" },
    ]
  );
  ...
  return (
    <div className="bg-white rounded-xl border border-slate-200 flex flex-col" style={{ height: tall ? "calc(100vh - 260px)" : 420 }}>
      <div className="px-5 py-3.5 border-b border-slate-100 flex items-center justify-between shrink-0">
        <div>
          <div className="text-xs font-semibold tracking-wide text-slate-400 uppercase">Communication</div>
          <div className="text-sm font-semibold text-slate-900">{vendor.name} · {vendor.company}</div>
        </div>
        <span className="inline-flex items-center gap-1.5 text-xs text-emerald-600">
          <span className="w-1.5 h-1.5 rounded-full bg-emerald-500" /> Vendor active
        </span>
      </div>
      <div className="flex-1 overflow-y-auto px-5 py-4 space-y-3">
        {messages.map((m, i) => (
          <div key={i} className={`flex ${m.from === "admin" ? "justify-end" : "justify-start"}`}>
            <div className={`rounded-xl px-3.5 py-2.5 text-sm ${m.from === "admin" ? "bg-emerald-600 text-white rounded-br-sm" : "bg-slate-100 text-slate-700 rounded-bl-sm"}`} style={{ maxWidth: "75%" }}>
              <div>{m.text}</div>
              <div className={`text-xs mt-1 ${m.from === "admin" ? "text-emerald-100" : "text-slate-400"}`}>{m.time}</div>
            </div>
          </div>
        ))}
        {typing && (
          <div className="flex justify-start">
            <div className="bg-slate-100 text-slate-400 rounded-xl rounded-bl-sm px-3.5 py-2.5 text-sm inline-flex items-center gap-1">
      ...
      <div className="p-3 border-t border-slate-100 shrink-0">
        <div className="flex items-end gap-2">
          <textarea
            ...
            rows={1}
            className="flex-1 resize-none rounded-lg border border-slate-200 text-sm px-3 py-2 outline-none focus:ring-2 focus:ring-emerald-500/30 focus:border-emerald-300"
            style={{ maxHeight: 96 }}
          />
          <button onClick={send} disabled={!draft.trim()} className="h-9 w-9 rounded-lg bg-emerald-600 text-white flex items-center justify-center hover:bg-emerald-700 disabled:opacity-40 shrink-0">
            <Send size={15} />
          </button>
        </div>
        <div className="flex flex-wrap gap-1.5 mt-2">
          <button onClick={() => setDraft(quickReplyFor(vendor, "docs"))} className="text-xs px-2 py-1 rounded-full bg-slate-100 text-slate-500 hover:bg-slate-200">Request missing documents</button>
          <button onClick={() => setDraft(quickReplyFor(vendor, "clarify"))} className="text-xs px-2 py-1 rounded-full bg-slate-100 text-slate-500 hover:bg-slate-200">Ask for clarification</button>
          <button onClick={() => setDraft(quickReplyFor(vendor, "approve"))} className="text-xs px-2 py-1 rounded-full bg-slate-100 text-slate-500 hover:bg-slate-200">Approve &amp; notify</button>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="p-6 space-y-5">
      ...
      <div className="flex bg-slate-100 rounded-lg p-0.5 text-sm font-medium w-fit">
        <button onClick={() => setTab("overview")} className={`px-4 py-1.5 rounded-md transition-colors ${tab === "overview" ? "bg-white text-slate-900 shadow-sm" : "text-slate-500 hover:text-slate-700"}`}>
          Overview
        </button>
        <button onClick={() => setTab("communication")} className={`px-4 py-1.5 rounded-md transition-colors inline-flex items-center gap-1.5 ${tab === "communication" ? "bg-white text-slate-900 shadow-sm" : "text-slate-500 hover:text-slate-700"}`}>
      ...
      {tab === "communication" && <CommunicationPanel vendor={vendor} tall />}
